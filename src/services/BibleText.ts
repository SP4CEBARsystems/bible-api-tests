...
import { BibleClient, PassageReference, type BibleBookHtml, type BibleCollection, type GetBooksItem, type GetTranslationsItem } from '@gracious.tech/fetch-client'

export default class BibleText {
    verses: string[];
    client: BibleClient;
    collection: BibleCollection|undefined;
    translations: GetTranslationsItem[] | undefined;
    translation_id: string|undefined;
    books: GetBooksItem[] | undefined;
    book: BibleBookHtml | undefined;
    corePromise: Promise<void>;
    ...
    async getVerse(refString:string = 'John 3:16-17', translationId:string = "eng_kjv"):Promise<string|undefined> {
        await this.corePromise;
        // return this.fetch();
        const ref = PassageReference.from_string(refString);
        console.log(ref);
        const book = await this.collection?.fetch_book(translationId, ref?.book??'');
        return book?.get_verse(ref?.start_chapter??1, ref?.start_verse??1);
    }
}